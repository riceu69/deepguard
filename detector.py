...
class DeepfakeDetector:
    ...
    def __init__(self, model_path='./models_v2/best_model.pth', use_pretrained=False):
        ...
        if not os.path.exists(model_path):
            print("⚠️  Model weights not found - using pretrained backbones only")
        ...
        if os.path.exists(model_path):
            ...
            print("⚠️  Using pretrained backbones only")
        
        self.model.to(self.device)
        self.model.eval()
        
        # Face detector - OpenCV Haar Cascade (lightweight!)
        print("Loading face detector (OpenCV)...")
        cascade_path = cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
        self.face_detector = cv2.CascadeClassifier(cascade_path)
        
        if self.face_detector.empty():
            print("⚠️  Could not load face detector cascade")
        else:
    ...
    def detect_faces(self, frames):
        """Detect and extract faces from frames using OpenCV"""
        faces = []
...
class FaceMatchingModel:
    ...
    def detect_and_crop_face(self, image):
        """Detect and crop face from PIL image using OpenCV"""
        img_array = np.array(image)
